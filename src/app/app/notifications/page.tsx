...
export default async function NotificationsPage() {
  ...
  return <section>
    <div className="appPageTitle notificationTitle"><div><p className="eyebrow">NOVEDADES</p><h1>Notificaciones</h1><p>Propuestas, oportunidades y cambios importantes.</p></div><div className="notificationTitleActions"><Link className="goldButton" href="/app/notifications/settings"><BellIcon/> Configurar push</Link>{items.some(n=>!n.is_read)&&<form action={markAllNotificationsRead}><button className="ghostButton">Marcar todas leídas</button></form>}</div></div>
    <div className="notificationList">{items.length?items.map(item=><form action={openNotification} key={item.id}><input type="hidden" name="id" value={item.id}/><input type="hidden" name="href" value={item.href||"/app"}/><button className={`notificationCard ${item.is_read?"read":"unread"}`}><span className="notificationIcon"><BellIcon/></span><span><strong>{item.title}</strong><small>{item.body}</small><em>{relativeTime(item.created_at)}</em></span></button></form>):<div className="emptyState"><h3>No tenés notificaciones</h3><p>Cuando aparezca una oportunidad compatible o cambie una propuesta, aparecerá acá.</p><Link className="primaryButton small" href="/app/notifications/settings">Crear una alerta push</Link></div>}</div>
  </section>;
}
